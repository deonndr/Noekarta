...
import { ChevronLeft, MapPin, Globe, ChevronDown } from 'lucide-react';
...
const LandmarkPage = () => {
    ...
    return (
        <div className="h-screen bg-white font-poppins flex flex-col overflow-hidden">
            <Seo
                title={t('landmark_sidebar_title')}
                description={t('landmark_sub')}
            />
            {/* Custom Header*/}
            ...
            <div className="flex flex-col lg:flex-row overflow-hidden" style={{ height: 'calc(100vh - 72px)' }}>
                ...
                <aside className="hidden lg:flex lg:w-[320px] xl:w-[380px] bg-white border-r border-gray-100 flex-col shrink-0 overflow-y-auto h-full">
                    <div className="p-6 border-b border-gray-100">
                        <h1 className="text-xl font-bold text-gray-900 mb-1">{t('landmark_sidebar_title')}</h1>
                        <p className="text-gray-500 text-sm">{t('landmark_sidebar_sub')}</p>
                    </div>

                    <div className="flex flex-col gap-2 p-4">
                        {landmarks.map((landmark) => {
                            const isActive = activeLandmark?.id === landmark.id;
                            const title = language === 'en' ? (landmark.title_en || landmark.title) : landmark.title;
                            const description = language === 'en' ? (landmark.description_en || landmark.description) : landmark.description;
                            return (
                                <button
                                    key={landmark.id}
                        ...
                        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[1000]">
                            <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-full shadow-[0_4px_20px_rgba(0,0,0,0.15)] border border-red-200">
                                ...
                                <span className="font-semibold text-gray-800 text-sm whitespace-nowrap">
                                    {language === 'en' ? (activeLandmark.title_en || activeLandmark.title) : activeLandmark.title}
                                </span>
                                <button
